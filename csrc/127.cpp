...
class Solution {
    bool areAdjacent(string word1, string word2) {
        int distance = 0;
    ...
    int bfs(int beginPos, int endPos, vector<string> &wordList) {
        bool beenTo[wordList.size()];
        int distance[wordList.size()];
        ...
        queue<int> q;
        q.push(beginPos);
        beenTo[beginPos] = true;
        distance[beginPos] = 1;
        while (!q.empty()) {
            int pos = q.front();
            q.pop();
            beenTo[pos] = true;
            if (pos == endPos) return distance[pos];
            for (int i = 0; i < wordList.size(); i++) {
                if (!beenTo[i] && areAdjacent(wordList[i], wordList[pos])) {
                    q.push(i);
                    beenTo[i] = true;
...
public:
    int ladderLength(const string &beginWord, const string &endWord, vector<string> &wordList) {
        ...
        int beginPos = -1;
        for (int i = 0; i < wordList.size(); ++i) {
            if (wordList[i] == endWord) endPos = i;
            if (wordList[i] == beginWord) beginPos = i;
        }
        if (endPos == -1)
            return 0;
        if (beginPos == -1) {
            wordList.push_back(beginWord);
            beginPos = wordList.size() - 1;
        }
        return bfs(beginPos, endPos, wordList);
    }
};
...
int main() {
    ...
    vector<string> *wordList = new vector<string>(words, words + sizeof(words) / sizeof(string));
    cout << solution.ladderLength("hit", "cog", *wordList) << endl;

    string newWords[] = {"hot", "dot", "dog", "lot", "log"};
    wordList = new vector<string>(newWords, newWords + sizeof(newWords) / sizeof(string));
    cout << solution.ladderLength("hit", "cog", *wordList) << endl;

    string newerWords[] = {"hot", "cog", "dog", "tot", "hog", "hop", "pot", "dot"};
    wordList = new vector<string>(newerWords, newerWords + sizeof(newerWords) / sizeof(string));
    cout << solution.ladderLength("hot", "dog", *wordList) << endl;

    string newestWords[] = {"ted", "tex", "red", "tax", "tad", "den", "rex", "pee"};
    wordList = new vector<string>(newestWords, newestWords + sizeof(newestWords) / sizeof(string));
    cout << solution.ladderLength("red", "tax", *wordList) << endl;

    string evenNewerWords[] = {"ricky", "grind", "cubic", "panic", "lover", "farce", "gofer", "sales", "flint", "omens",
    ...
    wordList = new vector<string>(evenNewerWords, evenNewerWords + sizeof(evenNewerWords) / sizeof(string));
    cout << solution.ladderLength("nanny", "aloud", *wordList) << endl;
}