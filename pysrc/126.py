...
class Solution:
    def isAdjacent(self, word1, word2) -> int:
        dist = 0
    ...
    def adjacent_list(self, wordList: List[str]) -> List[List[int]]:
        M = [[] for _ in range(len(wordList))]
    ...
    def bfs(self, M: List[List[int]], start: int, end: int) -> List[List[int]]:
        routes = {start: [[start]]}
        dists = [len(M)] * len(M)
        dists[start] = 0
        queue = [start]
        while len(queue) > 0:
            node = queue.pop(0)
            if dists[node] >= dists[end]:
                break
            for i in M[node]:
                if dists[i] > dists[node] + 1:
                    queue.append(i)
                    dists[i] = dists[node] + 1
                    routes[i] = [route + [i] for route in routes[node]]
                elif dists[i] == dists[node] + 1:
                    routes[i].extend([route + [i] for route in routes[node]])

        return routes[end] if end in routes else []
    ...
    def findLadders(self, beginWord: str, endWord: str, wordList: List[str]) -> List[List[str]]:
        ...
        if endWord not in wordList:
            return []
        d = {word: i for i, word in enumerate(wordList)}
        M = self.adjacent_list(wordList)
        routes = self.bfs(M, d[beginWord], d[endWord])
        ans = [[wordList[i] for i in route] for route in routes]
        return ans
...
if __name__ == '__main__':
    ...
    print(solution.findLadders(beginWord="red",
                               endWord="tax",
                               wordList=["ted", "tex", "red", "tax", "tad", "den", "rex", "pee"]))
    print(solution.findLadders(beginWord="nanny",
                               endWord="aloud",
                               wordList=["ricky", "grind", "cubic", "panic", "lover", "farce", "gofer", "sales",
